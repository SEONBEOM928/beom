...
//slice
//slice은 원본배열은 건드리지 않고 수정된복사 배열을 리턴한다(교체는 안됨)
//3,4를 출력
...
arr=arr.filter(function(item,index){ //filter안에 인자로 함수를 받고, index만 필요 하니까 명시

    return index !==delIndex; //배열을 돌면서 인덱스 1이아닌 나머지만 다시 소집함
});
